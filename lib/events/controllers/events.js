module.exports = function (init) {
  ...
  return {
    ...
    create: function (req, res) {
      ...
      if (!(event.organizerId = req.session.user.username))
        return res.reply(403, 'You are not authorized to edit this event');
  ...
  function fetch_event(req, cb, modify) {
    ...
    if (req.session.user && modify)
      ctx.loginAPI.getUserByUsername(req.session.user.username, function (err, user) {
        if (err || !user) {
  ...
  function list_events(req, view, requireAdmin) {
    ...
    function _where_query_builder(queryString) {
      var where = {};
      if (!queryString) {
        return where;
      }
      if (queryString.startRange && typeof queryString.startRange === 'string') {
        where.beginDate = {
          gte: new Date(queryString.startRange)
        }
      }
      if (queryString.endRange && typeof queryString.endRange === 'string') {
        where.endDate = {
          lte: new Date(queryString.endRange)
        }
      }
      if (queryString.whereAt && typeof queryString.whereAt === 'string') {
        where.address = {
          like: queryString.whereAt
        }
      }
      return where;
    }
    ...
    function _list_events(isAdmin) {
      if (req.query._page || req.query._limit) {
        ...
        var whereObj = _where_query_builder(req.query);
    ...
    if (req.session.user || requireAdmin)
      ctx.loginAPI.getUserByUsername(req.session.user.username, function (err, user) {
        if (err || !user)
          return res.reply(500, err || "User does not exist");
